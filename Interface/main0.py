...
def show_map():  #显示地图和小车的位置
    map = QPixmap("map.png")  #使用QPixmap获取图像
    ...
    ui.label_12.setPixmap(QPixmap("v4.png"));ui.label_12.setScaledContents(True)
    ui.label_13.setPixmap(QPixmap("v5.png"));ui.label_13.setScaledContents(True)
...
def show_movingcars():
    ...
    while n:
        #读取位置文件
	lo= open('/home/tan/ROBOTS/robot2/location.txt','r+')
	posex0 = lo.read(6)
        posey0=lo.readline()
        posey0=lo.read(6)
	posex1 = int(round((100*float(posex0))-100))
	posey1 = int(round((-95.2*float(posey0))+1358))
        ui.textBrowser_9.setText("计时：" + str(minute) + ':' + str(second))
        ui.label_13.setGeometry(QtCore.QRect(posex1,posey1,41,31))
        #可以在此插入show_error1()
